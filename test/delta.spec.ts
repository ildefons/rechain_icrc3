...
describe('Delta', () => {
    ...
    beforeAll(async () => {
      ...
      const fixture = await TestCan(pic, [jo.getPrincipal(), bob.getPrincipal()]);
      can = fixture.actor;
    ...
    it('upgrade canister', async () => {
      ...
      await passTime(10);
      await pic.upgradeCanister({ canisterId: canCanisterId, wasm: WASM_PATH, arg: IDL.encode(init({ IDL }), [{archive_controllers: [jo.getPrincipal(), bob.getPrincipal() ]}]) });
    ...
    it("Check canister controllers", async () => {
      ...
      for (let i=0; i<archives.length; i++) {
        ...
        mgr.setPrincipal(canCanisterId);
        let aset = await mgr.canister_status({canister_id: archives[i].canister_id});
